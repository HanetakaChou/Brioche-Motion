...
inline brx_animation_skeleton_mapper_ragdoll_direct_mapping_data::brx_animation_skeleton_mapper_ragdoll_direct_mapping_data(uint32_t joint_index_a, uint32_t joint_index_b, DirectX::XMFLOAT4X4 a_to_b_transform_model_space) : m_joint_index_a(joint_index_a), m_joint_index_b(joint_index_b), m_a_to_b_transform_model_space(a_to_b_transform_model_space)
{
}
...
inline void brx_animation_skeleton_mapper_ragdoll_mapping_data::set_direct_mappings(mcrt_vector<brx_animation_skeleton_mapper_ragdoll_direct_mapping_data> &&direct_mappings)
{
    this->m_direct_mappings = std::move(direct_mappings);
}

inline void brx_animation_skeleton_mapper_ragdoll_mapping_data::set_chain_mappings(mcrt_vector<brx_animation_skeleton_mapper_ragdoll_chain_mapping_data> &&chain_mappings)
{
    this->m_chain_mappings = std::move(chain_mappings);
}

inline void brx_animation_skeleton_mapper_ragdoll_mapping_data::set_unmapped(mcrt_vector<brx_animation_skeleton_mapper_ragdoll_unmapped_data> &&unmapped)
{
    this->m_unmapped = std::move(unmapped);
}

inline void brx_animation_skeleton_mapper_ragdoll_mapping_data::set_locked_translations(mcrt_vector<brx_animation_skeleton_mapper_ragdoll_locked_translation_data> &&locked_translations)
{
    this->m_locked_translations = std::move(locked_translations);
}

inline mcrt_vector<brx_animation_skeleton_mapper_ragdoll_direct_mapping_data> const &brx_animation_skeleton_mapper_ragdoll_mapping_data::get_direct_mappings() const
{
    return this->m_direct_mappings;
}

inline mcrt_vector<brx_animation_skeleton_mapper_ragdoll_chain_mapping_data> const &brx_animation_skeleton_mapper_ragdoll_mapping_data::get_chain_mappings() const
{
...
{
    uint32_t const skeleton_a_joint_count = in_skeleton_a.get_joint_count();
    ...
    mcrt_vector<DirectX::XMMATRIX> bind_pose_a_model_space(static_cast<size_t>(skeleton_a_joint_count));
    {
        ...
        {
            brx_motion_skeleton_joint_transform skeleton_a_joint_transform_local_space = in_skeleton_a.get_bind_pose_joint_transform_local_space(skeleton_a_joint_index);
            ...
            uint32_t const skeleton_a_parent_joint_index = in_skeleton_a.get_joint_parent_index(skeleton_a_joint_index);
            if (BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != skeleton_a_parent_joint_index)
            {
    ...
    uint32_t const skeleton_b_joint_count = in_skeleton_b.get_joint_count();
    ...
    mcrt_vector<DirectX::XMMATRIX> bind_pose_b_model_space(static_cast<size_t>(skeleton_b_joint_count));
    {
        ...
        {
            brx_motion_skeleton_joint_transform skeleton_b_joint_transform_local_space = in_skeleton_b.get_bind_pose_joint_transform_local_space(skeleton_b_joint_index);
            ...
            uint32_t const skeleton_b_parent_joint_index = in_skeleton_b.get_joint_parent_index(skeleton_b_joint_index);
            if (BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != skeleton_b_parent_joint_index)
            {
    ...
    mcrt_vector<brx_animation_skeleton_mapper_ragdoll_direct_mapping_data> direct_mappings;
    for (uint32_t skeleton_b_joint_index = 0; skeleton_b_joint_index < skeleton_b_joint_count; ++skeleton_b_joint_index)
    {
        uint32_t skeleton_a_joint_index = in_user_callback_map_joint_b_to_a(in_skeleton_a, in_skeleton_b, skeleton_b_joint_index, in_user_data_map_joint_b_to_a);
        if (BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != skeleton_a_joint_index)
        {
    ...
    mcrt_vector<brx_animation_skeleton_mapper_ragdoll_chain_mapping_data> chain_mappings;
    {
        mcrt_vector<mcrt_vector<uint32_t>> skeleton_b_children_joint_indices(static_cast<size_t>(skeleton_b_joint_count));
        mcrt_vector<mcrt_unordered_set<uint32_t>> skeleton_b_children_joint_indices_set(static_cast<size_t>(skeleton_b_joint_count));
        ...
        for (uint32_t skeleton_b_joint_index = 0; skeleton_b_joint_index < skeleton_b_joint_count; ++skeleton_b_joint_index)
        {
            uint32_t skeleton_b_parent_joint_index = in_skeleton_b.get_joint_parent_index(skeleton_b_joint_index);
            if (BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != skeleton_b_parent_joint_index)
            {
        ...
        mcrt_vector<bool> skeleton_b_joint_visited_flags(static_cast<size_t>(skeleton_b_joint_count), false);
        mcrt_vector<bool> skeleton_b_joint_pushed_flags(static_cast<size_t>(skeleton_b_joint_count), false);
        while (!skeleton_b_depth_first_search_stack.empty())
        {
            ...
            uint32_t skeleton_a_current_joint_index = in_user_callback_map_joint_b_to_a(in_skeleton_a, in_skeleton_b, skeleton_b_current_joint_index, in_user_data_map_joint_b_to_a);
            ...
            if (BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != skeleton_a_current_joint_index)
            {
...
#ifndef NDEBUG
                    // Whether the chain exists in A too
                    {
                        uint32_t const chain_b_start_joint_index = skeleton_b_depth_first_search_ancestor_map_reverse_skeleton_a[skeleton_b_depth_first_search_chain_start_ancestor_map_index].second;
                        uint32_t const chain_b_end_joint_index = skeleton_a_current_joint_index;
                        assert(BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != chain_b_start_joint_index);
                        assert(BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != chain_b_end_joint_index);
                        uint32_t chain_a_current_joint_index = chain_b_end_joint_index;
                        while ((BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != chain_a_current_joint_index) && (chain_b_start_joint_index != chain_a_current_joint_index))
                        {
                            chain_a_current_joint_index = in_skeleton_a.get_joint_parent_index(chain_a_current_joint_index);
                        }
                        assert(chain_b_start_joint_index == chain_a_current_joint_index);
                    }
#endif
                    chain_mappings.emplace_back(std::move(chain_b));
                }
    ...
    mcrt_vector<brx_animation_skeleton_mapper_ragdoll_unmapped_data> unmapped;
    for (uint32_t skeleton_b_joint_index = 0; skeleton_b_joint_index < skeleton_b_joint_count; ++skeleton_b_joint_index)
    {
        if (!mapped_b[skeleton_b_joint_index])
        {
            unmapped.emplace_back(skeleton_b_joint_index);
        }
    }

    mcrt_vector<brx_animation_skeleton_mapper_ragdoll_locked_translation_data> locked_translations;
    if ((in_lock_translations) && (direct_mappings.size() >= 1U))
    {
        uint32_t const root_joint_index_b = direct_mappings[0].get_joint_index_b();

#ifndef NDEBUG
        uint32_t root_joint_index_a = direct_mappings[0].get_joint_index_a();

        for (uint32_t ragdoll_direct_mapping_data_index = 1U; ragdoll_direct_mapping_data_index < direct_mappings.size(); ++ragdoll_direct_mapping_data_index)
        {
            uint32_t const joint_index_a = direct_mappings[ragdoll_direct_mapping_data_index].get_joint_index_a();
            uint32_t const joint_index_b = direct_mappings[ragdoll_direct_mapping_data_index].get_joint_index_b();
            assert(joint_index_a > root_joint_index_a);
            assert(joint_index_b > root_joint_index_b);
        }
#endif
        mcrt_vector<bool> locked(static_cast<size_t>(skeleton_b_joint_count), false);

        locked[root_joint_index_b] = true;

        // "root_joint_index_b" NOT included
        for (uint32_t skeleton_b_joint_index = (root_joint_index_b + 1U); skeleton_b_joint_index < skeleton_b_joint_count; ++skeleton_b_joint_index)
        {
            uint32_t const skeleton_b_parent_joint_index = in_skeleton_b.get_joint_parent_index(skeleton_b_joint_index);
            if (BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != skeleton_b_parent_joint_index)
            {
                ...
                if (locked[skeleton_b_parent_joint_index])
                {
                    ...
                    locked_translations.emplace_back(skeleton_b_joint_index);
                }
            }
        }
    }

    assert(out_ragdoll_mapping_data.get_direct_mappings().empty());
    assert(out_ragdoll_mapping_data.get_chain_mappings().empty());
    assert(out_ragdoll_mapping_data.get_unmapped().empty());
    assert(out_ragdoll_mapping_data.get_locked_translations().empty());
    out_ragdoll_mapping_data.set_direct_mappings(std::move(direct_mappings));
    out_ragdoll_mapping_data.set_chain_mappings(std::move(chain_mappings));
    out_ragdoll_mapping_data.set_unmapped(std::move(unmapped));
    out_ragdoll_mapping_data.set_locked_translations(std::move(locked_translations));
}
...
extern void brx_animation_skeleton_mapper_map_pose_model_space(brx_animation_skeleton_mapper_ragdoll_mapping_data const &in_ragdoll_mapping_data, DirectX::XMFLOAT4X4 const *const in_pose_a_model_space, uint32_t const in_skeleton_b_joint_count, uint32_t const *const in_skeleton_b_joint_parent_indices, DirectX::XMFLOAT4X4 const *const in_pose_b_local_space, DirectX::XMFLOAT4X4 *const out_pose_b_model_space)
{
    ...
    for (brx_animation_skeleton_mapper_ragdoll_chain_mapping_data const &ragdoll_chain_mapping_data : in_ragdoll_mapping_data.get_chain_mappings())
    {
        DirectX::XMFLOAT3 desired_end;
        {
            ...
            DirectX::XMStoreFloat3(&desired_end, end_joint_b_model_space_translation);
        ...
        uint32_t const chain_joint_count_b = ragdoll_chain_mapping_data.get_joint_indices_b().size();
        ...
        {
            {
                ...
                size_t space = size + (alignment - 1U);
                void *ptr = internal_stack_alloc(space);
                inout_chain_b_model_space = static_cast<DirectX::XMFLOAT4X4 *>(std::align(alignment, size, ptr, space));
            }
            ...
            for (uint32_t chain_index_b = 1U; chain_index_b < chain_joint_count_b; ++chain_index_b)
            {
                uint32_t const intermediate_joint_index_b = ragdoll_chain_mapping_data.get_joint_indices_b()[chain_index_b];

                chain_end = DirectX::XMMatrixMultiply(DirectX::XMLoadFloat4x4(&in_pose_b_local_space[intermediate_joint_index_b]), chain_end);

                DirectX::XMStoreFloat4x4(&inout_chain_b_model_space[chain_index_b], chain_end);
            }
        }

        ik_reaching_solve(desired_end, chain_joint_count_b, inout_chain_b_model_space);

        for (uint32_t chain_index_b_plus_1 = 2U; chain_index_b_plus_1 < chain_joint_count_b; ++chain_index_b_plus_1)
        {
...
#ifndef NDEBUG
            assert(!mapped_b[intermediate_joint_index_b]);
    ...
    for (brx_animation_skeleton_mapper_ragdoll_unmapped_data const &ragdoll_unmapped_data : in_ragdoll_mapping_data.get_unmapped())
    {
        ...
        if (BRX_MOTION_SKELETON_JOINT_INDEX_INVALID != parent_joint_index_b)
        {
#ifndef NDEBUG
