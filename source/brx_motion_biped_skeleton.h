...
#ifndef _BRX_MOTION_BIPED_SKELETON_H_
#define _BRX_MOTION_BIPED_SKELETON_H_ 1

#include "../include/brx_motion.h"
#include "../../McRT-Malloc/include/mcrt_vector.h"
#include "../../McRT-Malloc/include/mcrt_string.h"

class brx_motion_biped_skeleton final : public brx_motion_skeleton
{
	mcrt_vector<mcrt_string> m_skeleton_joint_names;
	mcrt_vector<int32_t> m_skeleton_joint_parent_indices;
	mcrt_vector<brx_motion_rigid_transform> m_skeleton_bind_pose_joint_transforms;

public:
	brx_motion_biped_skeleton();
	~brx_motion_biped_skeleton();
	void init(uint32_t skeleton_joint_count, char const *const *skeleton_joint_names, uint32_t const *skeleton_joint_parent_indices, brx_motion_rigid_transform const *skeleton_bind_pose_joint_transforms, uint32_t const vrm_skeleton_joint_indices[BRX_MOTION_VRM_SKELETON_JOINT_NAME_COUNT]);
	void uninit();
	...
	brx_motion_rigid_transform const *get_skeleton_bind_pose_joint_transforms() const override;
};

class brx_motion_biped_skeleton_animation final : public brx_motion_skeleton_animation
{
	mcrt_vector<mcrt_string> m_skeleton_joint_names;
	// Model Space
	mcrt_vector<brx_motion_rigid_transform> m_skeleton_animation_joint_transforms;
	uint32_t m_animation_frame_rate;

public:
	brx_motion_biped_skeleton_animation();
	~brx_motion_biped_skeleton_animation();
	void init(uint32_t skeleton_joint_count, char const *const *skeleton_joint_names, uint32_t animation_frame_rate, uint32_t animation_frame_count, brx_motion_rigid_transform const *skeleton_animation_joint_transforms);
	void uninit();
	uint32_t get_skeleton_joint_count() const;
	mcrt_vector<mcrt_string> const *get_skeleton_joint_names() const;
	uint32_t get_animation_frame_count() const;
	brx_motion_rigid_transform const *get_skeleton_animation_joint_transforms() const;
	uint32_t get_animation_frame_rate() const;
};

class brx_motion_biped_skeleton_animation_instance final : public brx_motion_skeleton_animation_instance
{
	mcrt_vector<brx_motion_rigid_transform> m_skeleton_animation_joint_transforms;
	uint32_t m_animation_frame_rate;
...
public:
	brx_motion_biped_skeleton_animation_instance();
	~brx_motion_biped_skeleton_animation_instance();
...
private:
	...
	brx_motion_rigid_transform const *get_skeleton_animation_joint_transforms() const override;
};
