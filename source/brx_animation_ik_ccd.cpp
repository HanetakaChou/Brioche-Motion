...
static inline void internal_ik_ccd_solve_iteration(float const in_gain, DirectX::XMFLOAT3 const &in_target_position_model_space, DirectX::XMFLOAT4X4 const &in_end_effector_transform_local_space, DirectX::XMFLOAT4X4 const &in_base_parent_transform_model_space, uint32_t const in_chain_joint_count, DirectX::XMFLOAT4X4 *const inout_chain_local_space, DirectX::XMFLOAT4X4 *const inout_chain_model_space);

extern void ik_ccd_solve(uint32_t const in_iteration, float const in_gain, DirectX::XMFLOAT3 const &in_target_position_model_space, DirectX::XMFLOAT4X4 const &in_end_effector_transform_local_space, uint32_t const in_chain_joint_count, DirectX::XMFLOAT4X4 *const inout_chain_local_space, DirectX::XMFLOAT4X4 *const inout_chain_model_space)
{
    DirectX::XMFLOAT4X4 in_base_parent_transform_model_space;
    assert(in_chain_joint_count >= 1U);
    {
        DirectX::XMVECTOR unused_determinant;
        DirectX::XMStoreFloat4x4(&in_base_parent_transform_model_space, DirectX::XMMatrixMultiply(DirectX::XMMatrixInverse(&unused_determinant, DirectX::XMLoadFloat4x4(&inout_chain_local_space[0])), DirectX::XMLoadFloat4x4(&inout_chain_model_space[0])));
    }

    for (uint32_t iteration_index = 0U; iteration_index < in_iteration; ++iteration_index)
    {
        internal_ik_ccd_solve_iteration(in_gain, in_target_position_model_space, in_end_effector_transform_local_space, in_base_parent_transform_model_space, in_chain_joint_count, inout_chain_local_space, inout_chain_model_space);
    }
}

static inline void internal_ik_ccd_solve_iteration(float const in_gain, DirectX::XMFLOAT3 const &in_target_position_model_space, DirectX::XMFLOAT4X4 const &in_end_effector_transform_local_space, DirectX::XMFLOAT4X4 const &in_base_parent_transform_model_space, uint32_t const in_chain_joint_count, DirectX::XMFLOAT4X4 *const inout_chain_local_space, DirectX::XMFLOAT4X4 *const inout_chain_model_space)
{
    for (uint32_t current_joint_chain_index_plus_1 = in_chain_joint_count; current_joint_chain_index_plus_1 >= 1U; --current_joint_chain_index_plus_1)
    {
        ...
        DirectX::XMVECTOR from_end_effector_to_target_model_space_rotation;
        {
            ...
            {
                uint32_t const end_effector_parent_chain_index = in_chain_joint_count - 1U;

                DirectX::XMMATRIX in_end_effector_transform_model_space = DirectX::XMMatrixMultiply(DirectX::XMLoadFloat4x4(&in_end_effector_transform_local_space), DirectX::XMLoadFloat4x4(&inout_chain_model_space[end_effector_parent_chain_index]));
        ...
        DirectX::XMStoreFloat4x4(&inout_chain_model_space[current_joint_chain_index], updated_current_joint_model_space_transform);
        ...
        DirectX::XMVECTOR current_joint_local_space_translation;
        {
            ...
            bool directx_xm_matrix_decompose = DirectX::XMMatrixDecompose(&current_joint_local_space_scale, &current_joint_local_space_rotation, &current_joint_local_space_translation, DirectX::XMLoadFloat4x4(&inout_chain_local_space[current_joint_chain_index]));
            assert(directx_xm_matrix_decompose);
        ...
        {
            DirectX::XMMATRIX current_parent_joint_model_space = (current_joint_chain_index >= 1U) ? DirectX::XMLoadFloat4x4(&inout_chain_model_space[current_joint_chain_index - 1U]) : DirectX::XMLoadFloat4x4(&in_base_parent_transform_model_space);
        ...
        DirectX::XMStoreFloat4x4(&inout_chain_local_space[current_joint_chain_index], updated_current_joint_local_space_transform);
        ...
        for (uint32_t child_joint_chain_index_plus_1 = (current_joint_chain_index_plus_1 + 1U); child_joint_chain_index_plus_1 <= in_chain_joint_count; ++child_joint_chain_index_plus_1)
        {
            ...
            DirectX::XMStoreFloat4x4(&inout_chain_model_space[child_joint_chain_index], DirectX::XMMatrixMultiply(DirectX::XMLoadFloat4x4(&inout_chain_local_space[child_joint_chain_index]), DirectX::XMLoadFloat4x4(&inout_chain_model_space[parent_joint_chain_index])));
        }
    }
}
