...
brx_motion_media_pipe_video_detector::brx_motion_media_pipe_video_detector() : m_ref_count(0U), m_face_count(0U), m_pose_count(0U), m_face_landmarker(NULL), m_pose_landmarker(NULL), m_timestamp_ms(0), m_enable_debug_renderer(false), m_named_window(NULL), m_input_video_capture(NULL)
{
}
...
void brx_motion_media_pipe_video_detector::uninit()
{
    ...
    {
        assert(NULL != this->m_named_window);
        internal_brx_destroy_window(this->m_named_window);
        this->m_named_window = NULL;
...
void brx_motion_media_pipe_video_detector::set_enable_debug_renderer(bool enable_debug_renderer, char const *debug_renderer_window_name)
{
    if (enable_debug_renderer != this->m_enable_debug_renderer)
    {
        if (enable_debug_renderer)
        {
            ...
            assert(NULL == this->m_named_window);
            this->m_named_window = internal_brx_named_window(this->m_debug_renderer_window_name.c_str());
        }
        else
        {
            assert(NULL != this->m_named_window);
    ...
    if (!video_frame->empty())
    {
...

        {
            ...
            if (this->m_enable_debug_renderer)
            {
                ...
                assert(NULL != this->m_named_window);
                internal_brx_image_show(this->m_named_window, image_buffer, image_width, image_height);
            }
        }
