...
enum
{
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_RIGHT_ARM = 0,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_RIGHT_ELBOW = 1,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_RIGHT_WRIST = 2,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_RIGHT_INDEX = 3,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_RIGHT_LEG = 4,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_RIGHT_KNEE = 5,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_RIGHT_ANKLE = 6,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_RIGHT_TOE_TIP = 7,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_LEFT_ARM = 8,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_LEFT_ELBOW = 9,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_LEFT_WRIST = 10,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_LEFT_INDEX = 11,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_LEFT_LEG = 12,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_LEFT_KNEE = 13,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_LEFT_ANKLE = 14,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_LEFT_TOE_TIP = 15,
    INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_COUNT = 16
};
...
brx_motion_media_pipe_video_detector::brx_motion_media_pipe_video_detector() : m_face_count(0U), m_pose_count(0U), m_face_landmarker(NULL), m_pose_landmarker(NULL), m_timestamp_ms(0), m_enable_debug_renderer(false), m_input_video_capture(NULL)
{
}
...
void brx_motion_media_pipe_video_detector::set_input(brx_motion_video_capture const *video_capture)
{
    assert(this->m_input_video_capture != video_capture);
...

        {
            ...
            if ((this->m_face_count > 0U) && (0 == status_face_landmarker_detect_for_video))
            {
                ...
                for (uint32_t face_index = 0U; face_index < face_landmarker_result.face_blendshapes_count && face_index < this->m_face_count; ++face_index)
                {
                    ...
                    DirectX::XMFLOAT4 left_eye_rotation;
                    {
                        ...
                        DirectX::XMVECTOR const eye_rotation_up = DirectX::XMQuaternionRotationAxis(DirectX::XMLoadFloat3(&axis_right), 0.0F - DirectX::XM_PI * INTERNAL_EYE_ANGLE_MAXIMUM);
                        ...
                        DirectX::XMVECTOR const eye_rotation_left = DirectX::XMQuaternionRotationAxis(DirectX::XMLoadFloat3(&axis_up), 0.0F - DirectX::XM_PI * INTERNAL_EYE_ANGLE_MAXIMUM);
                ...
                for (uint32_t face_index = 0U; face_index < face_landmarker_result.facial_transformation_matrixes_count && face_index < this->m_face_count; ++face_index)
                {
                    ...
                    DirectX::XMFLOAT4 mmd_head_rotation;
                    {
                        // MediaPipe Face
                        // RH
                        // Right -X
                        ...
                        constexpr DirectX::XMFLOAT4X4 const mmd_to_media_pipe(
                            1.0F, 0.0F, 0.0F, 0.0F,
                            0.0F, 1.0F, 0.0F, 0.0F,
                            0.0F, 0.0F, -1.0F, 0.0F,
                            0.0F, 0.0F, 0.0F, 1.0F);

                        constexpr DirectX::XMFLOAT4X4 const media_pipe_to_mmd(
                            1.0F, 0.0F, 0.0F, 0.0F,
                            0.0F, 1.0F, 0.0F, 0.0F,
                            0.0F, 0.0F, -1.0F, 0.0F,
                            0.0F, 0.0F, 0.0F, 1.0F);

                        DirectX::XMMATRIX mmd_head_transform = DirectX::XMMatrixMultiply(DirectX::XMMatrixMultiply(DirectX::XMLoadFloat4x4(&mmd_to_media_pipe), DirectX::XMLoadFloat4x4(&media_pipe_head_transform)), DirectX::XMLoadFloat4x4(&media_pipe_to_mmd));

                        constexpr float const INTERNAL_SCALE_EPSILON = 1E-4F;

                        DirectX::XMVECTOR simd_mmd_head_scale;
                        DirectX::XMVECTOR simd_mmd_head_rotation;
                        DirectX::XMVECTOR simd_mmd_head_translation;
                        bool directx_xm_matrix_decompose = DirectX::XMMatrixDecompose(&simd_mmd_head_scale, &simd_mmd_head_rotation, &simd_mmd_head_translation, mmd_head_transform);
                        assert(directx_xm_matrix_decompose);

                        assert(DirectX::XMVector3EqualInt(DirectX::XMVectorTrueInt(), DirectX::XMVectorLess(DirectX::XMVectorAbs(DirectX::XMVectorSubtract(simd_mmd_head_scale, DirectX::XMVectorSplatOne())), DirectX::XMVectorReplicate(INTERNAL_SCALE_EPSILON))));

                        DirectX::XMStoreFloat4(&mmd_head_rotation, simd_mmd_head_rotation);
                    }

                    face_skeleton_joint_rotations[INTERNAL_MEIDA_PIPE_FACE_SKELETON_JOINT_NAME_HEAD] = mmd_head_rotation;
                }
            ...
            if ((this->m_pose_count > 0U) && (0 == status_pose_landmarker_detect_for_video))
            {
                for (uint32_t pose_index = 0U; pose_index < this->m_pose_count && pose_index < pose_landmarker_result.pose_world_landmarks_count; ++pose_index)
                {
                    ...
                    DirectX::XMMATRIX simd_media_pipe_to_mmd = DirectX::XMLoadFloat4x4(&media_pipe_to_mmd);
                    ...
                    static_assert(INTERNAL_MEIDA_PIPE_POSE_SKELETON_JOINT_NAME_COUNT == mapping_count, "");
                    ...
                    for (uint32_t mapping_index = 0U; mapping_index < mapping_count; ++mapping_index)
                    {
                        ...
                        DirectX::XMStoreFloat3(&pose_skeleton_joint_translations_model_space[skeleton_joint_index], DirectX::XMVector3TransformCoord(DirectX::XMLoadFloat3(&media_pipe_positions[position_index]), simd_media_pipe_to_mmd));
