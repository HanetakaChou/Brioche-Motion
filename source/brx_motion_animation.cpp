...
static uint32_t g_physics_context_reference_count = 0U;
...
static constexpr float const INTERNAL_FRAME_TIME = 1.0F / 30.0F;

static constexpr float const INTERNAL_PHYSICS_SYNCHRONIZATION_TIME = 1.0F;
...
extern "C" void brx_motion_destroy_animation(brx_motion_animation *wrapped_animation)
{
    ...
    brx_motion_animation_animation *delete_unwrapped_animation = static_cast<brx_motion_animation_animation *>(wrapped_animation);
...
extern "C" brx_motion_animation_instance *brx_motion_create_animation_instance(brx_motion_animation *wrapped_animation)
{
    ...
    brx_motion_animation_animation_instance *new_unwrapped_animation_instance = new (new_unwrapped_animation_instance_base) brx_motion_animation_animation_instance{static_cast<brx_motion_animation_animation *>(wrapped_animation)};
...
extern "C" void brx_motion_destroy_animation_instance(brx_motion_animation_instance *wrapped_animation_instance)
{
    ...
    brx_motion_animation_animation_instance *delete_unwrapped_animation_instance = static_cast<brx_motion_animation_animation_instance *>(wrapped_animation_instance);
...
extern "C" void brx_motion_destroy_skeleton(brx_motion_skeleton *wrapped_skeleton)
{
    ...
    brx_motion_animation_skeleton *delete_unwrapped_skeleton = static_cast<brx_motion_animation_skeleton *>(wrapped_skeleton);
...
extern "C" brx_motion_skeleton_instance *brx_motion_create_skeleton_instance(brx_motion_skeleton *wrapped_skeleton)
{
    ...
    brx_motion_animation_skeleton_instance *new_unwrapped_skeleton_instance = new (new_unwrapped_skeleton_instance_base) brx_motion_animation_skeleton_instance{static_cast<brx_motion_animation_skeleton *>(wrapped_skeleton)};

    new_unwrapped_skeleton_instance->init();
...
inline brx_motion_animation_animation_instance::brx_motion_animation_animation_instance(brx_motion_animation_animation *animation)
...
void brx_motion_animation_animation_instance::step(float delta_time)
{
    ...
    assert(this->m_remainder_time < INTERNAL_FRAME_TIME);
    this->m_remainder_time = (this->m_remainder_time + delta_time);

    uint32_t delta_frame_count;
    if (this->m_remainder_time >= INTERNAL_FRAME_TIME)
    {
        delta_frame_count = static_cast<uint32_t>(this->m_remainder_time * (1.0F / INTERNAL_FRAME_TIME));
        this->m_remainder_time = (this->m_remainder_time - INTERNAL_FRAME_TIME * delta_frame_count);
    }
    else
    {
    ...
    this->m_delta_time = INTERNAL_FRAME_TIME * delta_frame_count;
...
inline uint32_t brx_motion_animation_animation_instance::get_frame_index() const
{
...
inline brx_motion_animation_skeleton_instance::brx_motion_animation_skeleton_instance(brx_motion_animation_skeleton *skeleton) : m_skeleton(skeleton), m_physics_world(NULL), m_skin_transforms(static_cast<size_t>(skeleton->get_animation_skeleton_joint_count())), m_input_video_detector(NULL), m_input_face_index(BRX_MOTION_UINT32_INDEX_INVALID), m_input_pose_index(BRX_MOTION_UINT32_INDEX_INVALID), m_input_animation_instance(NULL), m_input_continuous(false)
{
}
...
inline void brx_motion_animation_skeleton_instance::init()
{
    uint32_t const ragdoll_skeleton_rigid_body_count = this->m_skeleton->get_ragdoll_skeleton_rigid_body_count();

    brx_animation_physics_rigid_body const *const ragdoll_skeleton_rigid_bodies = this->m_skeleton->get_ragdoll_skeleton_rigid_bodies();

    uint32_t const ragdoll_skeleton_constraint_count = this->m_skeleton->get_ragdoll_skeleton_constraint_count();

    brx_animation_physics_constraint const *const ragdoll_skeleton_constraints = this->m_skeleton->get_ragdoll_skeleton_constraints();

    // TODO: only initialized the context when module initialize
    // remove reference count
    {
        if (0U == g_physics_context_reference_count)
        {
            assert(NULL == g_physics_context);
            g_physics_context = brx_physics_create_context();
        }

        ++g_physics_context_reference_count;
    }
    assert(NULL != g_physics_context);
...
{
    uint32_t const ragdoll_skeleton_constraint_count = this->m_physics_constraints.size();

    for (uint32_t ragdoll_skeleton_constraint_index = 0U; ragdoll_skeleton_constraint_index < ragdoll_skeleton_constraint_count; ++ragdoll_skeleton_constraint_index)
    {
        assert(NULL != this->m_physics_constraints[ragdoll_skeleton_constraint_index]);
        brx_physics_world_remove_constraint(g_physics_context, this->m_physics_world, this->m_physics_constraints[ragdoll_skeleton_constraint_index]);

        brx_physics_destroy_constraint(g_physics_context, this->m_physics_world, this->m_physics_constraints[ragdoll_skeleton_constraint_index]);
    }

    uint32_t const ragdoll_skeleton_rigid_body_count = this->m_physics_rigid_bodies.size();

    for (uint32_t ragdoll_skeleton_rigid_body_index = 0U; ragdoll_skeleton_rigid_body_index < ragdoll_skeleton_rigid_body_count; ++ragdoll_skeleton_rigid_body_index)
    {
        assert(NULL != this->m_physics_rigid_bodies[ragdoll_skeleton_rigid_body_index]);
        brx_physics_world_remove_rigid_body(g_physics_context, this->m_physics_world, this->m_physics_rigid_bodies[ragdoll_skeleton_rigid_body_index]);

        brx_physics_destroy_rigid_body(g_physics_context, this->m_physics_world, this->m_physics_rigid_bodies[ragdoll_skeleton_rigid_body_index]);
    }
    ...
    assert(NULL != g_physics_context);
    {
        assert(g_physics_context_reference_count > 0U);
...
void brx_motion_animation_skeleton_instance::set_input(brx_motion_video_detector const *video_detector, uint32_t face_index, uint32_t pose_index)
{
    assert(this->m_input_video_detector != video_detector);
    ...
    constexpr uint32_t const video_detector_pose_skeleton_joint_count = (sizeof(this->m_video_detector_pose_skeleton_joint_rotations_local_space) / sizeof(this->m_video_detector_pose_skeleton_joint_rotations_local_space[0]));
    for (uint32_t video_detector_pose_skeleton_joint_index = 0U; video_detector_pose_skeleton_joint_index < video_detector_pose_skeleton_joint_count; ++video_detector_pose_skeleton_joint_index)
    {
        DirectX::XMStoreFloat4(&this->m_video_detector_pose_skeleton_joint_rotations_local_space[video_detector_pose_skeleton_joint_index], DirectX::XMQuaternionIdentity());
    }
...
void brx_motion_animation_skeleton_instance::set_input(brx_motion_animation_instance const *wrapped_animation_instance)
{
    ...
    assert(this->m_input_animation_instance != wrapped_animation_instance);
    this->m_input_animation_instance = static_cast<brx_motion_animation_animation_instance const *>(wrapped_animation_instance);

    this->m_input_continuous = false;
}

void brx_motion_animation_skeleton_instance::step()
{
    ...
    if (NULL != this->m_input_video_detector)
    {
        ...
        assert((NULL != input_video_detector) || ((BRX_MOTION_UINT32_INDEX_INVALID == this->m_input_face_index) && (BRX_MOTION_UINT32_INDEX_INVALID == this->m_input_pose_index)));
...

        {
            ...
            for (uint32_t video_detector_face_skeleton_joint_index = 0U; video_detector_face_skeleton_joint_index < video_detector_face_skeleton_joint_count; ++video_detector_face_skeleton_joint_index)
            {
                ...
                {
                    DirectX::XMFLOAT4 const *const current_rotation_local_space = video_detector_face_rotation_model_space;
                    ...
                    DirectX::XMStoreFloat4(&blend_rotation_local_space, DirectX::XMQuaternionSlerp(DirectX::XMLoadFloat4(&previous_rotation_local_space), DirectX::XMLoadFloat4(current_rotation_local_space), (1.0F - 0.125F)));
...

        {
            ...
            constexpr BRX_MOTION_SKELETON_JOINT_NAME const internal_video_detector_pose_skeleton_joints[][2] = {
            ...
            constexpr uint32_t const video_detector_pose_skeleton_joint_count = sizeof(internal_video_detector_pose_skeleton_joints) / sizeof(internal_video_detector_pose_skeleton_joints[0]);
            static_assert((sizeof(this->m_video_detector_pose_skeleton_joint_rotations_local_space) / sizeof(this->m_video_detector_pose_skeleton_joint_rotations_local_space[0])) == video_detector_pose_skeleton_joint_count, "");

            // use the previous rotation as the initial state for IK solver
            for (uint32_t video_detector_pose_skeleton_joint_index = 0U; video_detector_pose_skeleton_joint_index < video_detector_pose_skeleton_joint_count; ++video_detector_pose_skeleton_joint_index)
            {
                BRX_MOTION_SKELETON_JOINT_NAME const ball_and_socket_joint_animation_skeleton_joint_name = internal_video_detector_pose_skeleton_joints[video_detector_pose_skeleton_joint_index][0];
                ...
                {
                    DirectX::XMFLOAT4 const &previous_ball_and_socket_joint_rotation_local_space = this->m_video_detector_pose_skeleton_joint_rotations_local_space[video_detector_pose_skeleton_joint_index];
            ...
            {
                BRX_MOTION_SKELETON_JOINT_NAME const ball_and_socket_joint_animation_skeleton_joint_name = internal_video_detector_pose_skeleton_joints[video_detector_pose_skeleton_joint_index][0];
                BRX_MOTION_SKELETON_JOINT_NAME const end_effector_animation_skeleton_joint_name = internal_video_detector_pose_skeleton_joints[video_detector_pose_skeleton_joint_index][1];

                uint32_t const ball_and_socket_joint_animation_skeleton_joint_index = this->m_skeleton->get_animation_skeleton_joint_index(ball_and_socket_joint_animation_skeleton_joint_name);
                DirectX::XMFLOAT3 const *const ball_and_socket_joint_video_detector_pose_translation_model_space = input_video_detector->get_pose_skeleton_joint_translation(this->m_input_pose_index, ball_and_socket_joint_animation_skeleton_joint_name);
                ...
                DirectX::XMFLOAT3 const *const end_effector_video_detector_pose_translation_model_space = input_video_detector->get_pose_skeleton_joint_translation(this->m_input_pose_index, end_effector_animation_skeleton_joint_name);

                if ((BRX_MOTION_UINT32_INDEX_INVALID != ball_and_socket_joint_animation_skeleton_joint_index) && (NULL != ball_and_socket_joint_video_detector_pose_translation_model_space) && (BRX_MOTION_UINT32_INDEX_INVALID != end_effector_animation_skeleton_joint_index) && (NULL != end_effector_video_detector_pose_translation_model_space))
                {
                    ...
                    DirectX::XMStoreFloat4(&blend_ball_and_socket_joint_rotation_local_space, DirectX::XMQuaternionSlerp(DirectX::XMLoadFloat4(&previous_ball_and_socket_joint_rotation_local_space), DirectX::XMLoadFloat4(&current_ball_and_socket_joint_rotation_local_space), (1.0F - 0.125F)));

                    // TODO: use current or blend?
                    this->m_video_detector_pose_skeleton_joint_rotations_local_space[video_detector_pose_skeleton_joint_index] = blend_ball_and_socket_joint_rotation_local_space;
    ...
    if (NULL != this->m_input_animation_instance)
    {
        if (this->m_input_continuous)
        {
            ...
            {
                physics_delta_time = INTERNAL_PHYSICS_SYNCHRONIZATION_TIME;
            }
        }
        else
        {
    ...
    else
    {
        if (this->m_input_continuous)
        {
            physics_delta_time = INTERNAL_FRAME_TIME;
        }
        else
        {
            physics_delta_time = INTERNAL_PHYSICS_SYNCHRONIZATION_TIME;
    ...
    this->ragdoll(animation_skeleton_animation_pose_model_space, physics_delta_time);
...
inline void brx_motion_animation_skeleton_instance::ragdoll(mcrt_vector<DirectX::XMFLOAT4X4> &animation_skeleton_animation_pose_model_space, float physics_delta_time)
{
    //
    ...
    //
    brx_physics_world_step(g_physics_context, this->m_physics_world, physics_delta_time);
